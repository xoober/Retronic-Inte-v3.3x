...
#define REPORT_SIZE 4  //modified 202504
...
static char intellivisionBuildReport(unsigned char *reportBuffer, char id)
{
	...
	if (reportBuffer)
	{
		...
		switch(tmp&0x70)
		{
			...
			case 0b01110000:  but[1]=0b00000000; but[0]=0b00000011; break; //two buttons; added 202504

			default: but[1]=but[0]=0; //All buttons depressed
		}
		...
		switch(tmp)
		{
			...
			case 0b01011010: x = y = 0x80;  but[1]=0b00001000; but[0]=0b00001000; break; //[K1+K9] modified 202504
			case 0b01010101: x = y = 0x80;  but[1]=0b01000000; but[0]=0b01000000; break; // k4+kE added 202504
			case 0b00110101: x = y = 0x80;  but[1]=0b00010000; but[0]=0b10000000; break; // k5+kC added 202504
			case 0b01100101: x = y = 0x80;  but[1]=0b01000000; but[0]=0b10000000; break; // k5+kE added 202504
			case 0b00111010: x = y = 0x80;  but[1]=0b00000100; but[0]=0b00001000; break; // k1+k8 added 202504
			case 0b01101010: x = y = 0x80;  but[1]=0b00001000; but[0]=0b00010000; break; // k2+k9 added 202504
		...
		* [4] Raw controller input with no interpretation (8 bits)
		...
		//reportBuffer[4] = tmp;  removed 202504
...
const char intellivision_usbHidReportDescriptor[] PROGMEM = {
	...
	0x29, 15,               	   //   USAGE_MAXIMUM (Button 15), modified 202504
    0x15, 0x00,                    //   LOGICAL_MINIMUM (0)
    0x25, 0x01,                    //   LOGICAL_MAXIMUM (1)
    0x75, 1,                  	   //   REPORT_SIZE (1)
    //0x95, 24,                      //   REPORT_COUNT (24)
	0x95, 16,                      //   REPORT_COUNT (16), modified 202504
    0x81, 0x02,                    // INPUT (Data,Var,Abs)
